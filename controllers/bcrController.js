...
exports.listSubmissions = async (req, res) => {
  try {
    ...
    if (process.env.NODE_ENV === 'test') {
      ...
      // Log that we're querying the database
      ...
      if (submissions.length > 0) {
        moduleLogger.debug('First submission data:', JSON.stringify(submissions[0]));
      }
    }
    ...
    submissions = submissions.map(submission => {
      ...
      moduleLogger.debug(`Formatted submission: ${submissionObj.submissionCode}`);
      return submissionObj;
    });
    
    // Log the first submission for debugging
    if (submissions.length > 0) {
      moduleLogger.debug('First submission data after formatting:', JSON.stringify(submissions[0], null, 2));
    }
...
/**
 * Submission dashboard - specific handler for /bcr/submissions/dashboard
 */
exports.submissionDashboard = async (req, res) => {
  try {
    moduleLogger.info('Viewing submission dashboard', { userId: req.user?.id });
    
    // Redirect to the submissions list page
    res.redirect('/bcr/submissions');
  } catch (error) {
    moduleLogger.error('Error viewing submission dashboard', { error, userId: req.user?.id });
    res.status(500).render('error', {
      title: 'Error',
      message: 'An error occurred while loading the submission dashboard',
      error: process.env.NODE_ENV === 'development' ? error : {},
      user: req.user
    });
  }
