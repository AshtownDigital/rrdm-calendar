...
/**
 * RRDM Green Deployment Script
 * 
 * This script handles the deployment of the RRDM application from the green branch.
...
async function deploy() {
  try {
    console.log('🚀 Starting RRDM Green Deployment');
    
    // Ensure we're on the green branch
    checkBranch();
    ...
    console.log('✅ RRDM Green Deployment completed successfully!');
    console.log('');
...
/**
 * Check if we're on the green branch
 */
function checkBranch() {
  try {
    ...
    if (currentBranch !== config.branch) {
      console.log(`⚠️ Currently on branch '${currentBranch}', switching to '${config.branch}'`);
      execSync(`git checkout ${config.branch}`, { stdio: 'inherit' });
    }
    
    console.log(`✅ Using branch: ${config.branch}`);
  } catch (error) {
    throw new Error(`Failed to check or switch branch: ${error.message}`);
  }
}
