/**
 * Script to repopulate the BCR Phase-Status mapping in the database
 * This script creates the necessary phase and status configurations in the BcrConfigs table
 * with the correct relationships between phases and statuses
 */
...
async function createPhase(phase) {
  ...
  const phaseConfig = await prisma.bcrConfigs.create({
    data: {
      ...
      value: phase.value, // Phase value (e.g., "1", "2", etc.)
...
async function createStatus(status) {
  ...
  const statusConfig = await prisma.bcrConfigs.create({
    data: {
      ...
      value: status.phaseValue, // Link to the phase value (e.g., "1", "2", etc.)
 ...
 */
async function repopulatePhaseStatusMapping() {
  try {
    console.log('Starting to repopulate phase-status mapping...');
    ...
    for (const mapping of phaseStatusMapping) {
      const phase = await createPhase(mapping.phase);
      createdPhases[mapping.phase.value] = phase;
      console.log(`Created phase: ${phase.name} (ID: ${phase.id}, Value: ${phase.value})`);
    }
    ...
    for (const mapping of phaseStatusMapping) {
      ...
      const inProgressStatus = await createStatus({
      ...
      console.log(`Created in-progress status: ${inProgressStatus.name} (ID: ${inProgressStatus.id}, Value: ${inProgressStatus.value})`);
      statusDisplayOrder += 10;
      
      // Create completed status
      const completedStatus = await createStatus({
        name: mapping.completedStatus,
        phaseValue: mapping.phase.value,
        displayOrder: statusDisplayOrder,
        description: `Completed status for phase: ${mapping.phase.name}`
      });
      console.log(`Created completed status: ${completedStatus.name} (ID: ${completedStatus.id}, Value: ${completedStatus.value})`);
      statusDisplayOrder += 10;
    }
    ...
    for (const mapping of phaseStatusMapping) {
      ...
      const trelloMapping = await prisma.bcrConfigs.create({
        data: {
          id: uuidv4(),
      ...
      console.log(`Created Trello mapping: ${trelloMapping.name} (ID: ${trelloMapping.id}, Value: ${trelloMapping.value})`);
    ...
    console.log('Phase-status mapping repopulated successfully!');
  } catch (error) {
    console.error('Error repopulating phase-status mapping:', error);
  } finally {
