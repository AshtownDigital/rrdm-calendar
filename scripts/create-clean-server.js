/**
 * Script to create a clean server.js file focused on BCR functionality
 */
const fs = require('fs');
const path = require('path');

function createCleanServer() {
  console.log('Creating clean server.js file...');
  
  try {
    // Create a clean server.js file with just the BCR functionality
    const cleanServerContent = `/**
 * Clean server.js for RRDM application
 * Focused on BCR functionality
 */
...
module.exports = server;`;
    ...
    // Write the clean server.js file
    const cleanServerPath = path.join(__dirname, 'server.js.clean');
    fs.writeFileSync(cleanServerPath, cleanServerContent);
    
    console.log(`Created clean server.js at: ${cleanServerPath}`);
    console.log('To apply the clean server, run:');
    console.log(`cp ${cleanServerPath} ${path.join(__dirname, '..', 'server.js')}`);
    
    return true;
  } catch (error) {
    console.error('Error creating clean server.js:', error);
    return false;
  }
}
