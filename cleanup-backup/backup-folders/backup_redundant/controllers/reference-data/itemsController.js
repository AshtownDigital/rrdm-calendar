...
const getCurrentAcademicYear = async (req, allowAll = true) => {
  try {
    ...
    const academicYears = await ReferenceData.find({ category: 'academicYear' }).sort({ name: -1 });
...
const listItems = async (req, res) => {
  try {
    ...
    let items = await ReferenceData.find({ category: { $ne: 'academicYear' } }).sort({ name: 1 });
...
const viewItemValues = async (req, res) => {
  try {
    ...
    const item = await ReferenceData.findById(itemId);
    ...
    const values = await ReferenceValue.find({ referenceDataId: itemId }).sort({ name: 1 });
...
const viewItemHistory = async (req, res) => {
  try {
    ...
    // Get the item from the database
    const item = await ReferenceData.findById(itemId).populate('history');
