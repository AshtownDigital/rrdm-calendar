 ...
 */
const { FundingAllocation, FundingData } = require('../../models');
...
const listAllocations = async (req, res) => {
  try {
    ...
    const allocations = await FundingAllocation.find().sort({ createdAt: -1 });
...
const showAddAllocationForm = async (req, res) => {
  try {
    ...
    const fundingItems = await FundingData.find().sort({ name: 'asc' });
...
const processAddAllocation = async (req, res) => {
  try {
    const { fundingItemId, amount, description, startDate, endDate } = req.body;
    
    // Validation
    if (!fundingItemId || !amount || !description || !startDate || !endDate) {
      req.flash('error_msg', 'Please fill in all fields');
      return res.redirect('/funding/allocations/add');
    }
    
    // Create allocation
    await FundingAllocation.create({
      fundingItemId,
      amount,
      description,
...
const showEditAllocationForm = async (req, res) => {
  try {
    ...
    const allocation = await FundingAllocation.findById(allocationId);
...
const updateAllocation = async (req, res) => {
  try {
    ...
    // Update allocation
    await FundingAllocation.findByIdAndUpdate(allocationId, {
      fundingItemId,
      amount,
      description,
      startDate: new Date(startDate),
...
const deleteAllocation = async (req, res) => {
  try {
    ...
    // Delete allocation
    await FundingAllocation.findByIdAndDelete(allocationId);
