...
async function submitBCRUpdate(req, res) {
  try {
    ...
    let bcr = null;
    try {
      bcr = await Bcr.findById(submissionId);
    } catch (error) {
    ...
    if (phaseId === 'reject') {
      ...
      await Bcr.findByIdAndUpdate(submissionId, {
        status: 'Rejected',
    ...
    try {
      ...
      phaseConfig = await BcrConfig.findOne({
        type: 'phase',
      ...
      status = await BcrConfig.findOne({
        type: 'status',
        value: statusType
      });
    } catch (error) {
    ...
    if (phaseCompleted === 'true') {
      ...
      try {
        completedStatus = await BcrConfig.findOne({
          type: 'status',
      ...
      try {
        // Get the next phase
        nextPhase = await BcrConfig.findOne({
          type: 'phase',
        ...
        if (nextPhase) {
          nextPhaseInProgressStatus = await BcrConfig.findOne({
            type: 'status',
      ...
      await Bcr.findByIdAndUpdate(id, {
