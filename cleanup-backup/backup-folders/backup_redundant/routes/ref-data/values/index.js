...
router.get('/', (req, res) => {
      ...
      if (item) {
        ...
        itemValues.forEach(value => {
          valuesList.push({
            ...
            actions: `<a href="/ref-data/values/${itemId}/${value.id}/history" class="govuk-link">History</a>`
          });
        });
      }
    });
  ...
  res.render('modules/ref-data/values/index', {
...
router.get('/sex', (req, res) => {
  ...
  if (currentYear.id === 'all') {
    ...
    formattedValues = itemValues.map(value => ({
      ...
      actions: `<a href="/ref-data/values/${itemId}/${value.id}/history" class="govuk-button govuk-button--secondary">View History</a>`
    }));
  } else {
    // For specific year view, filter by the selected academic year
  ...
  res.render('modules/ref-data/values/sex', {
...
router.get('/:itemId', (req, res) => {
  ...
  const formattedValues = itemValues.map(value => ({
    ...
    changeType: value.changeType,
    changeSummary: value.history && value.history[0] ? value.history[0].summary : '',
    actions: `<a href="/ref-data/values/${item.id}/${value.id}/history" class="govuk-button govuk-button--secondary">View History</a>`
  }));

  res.render('modules/ref-data/values/list', {
    title: `${item.name} Values`,
    item,
    values: formattedValues
  });
...
router.get('/:itemId/:valueId/history', (req, res) => {
  ...
  res.render('modules/ref-data/values/history', {
