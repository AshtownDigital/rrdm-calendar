...
async function getAllReleases(filterOptions = {}) {
  try {
    console.log('Service: getAllReleases called with filterOptions:', JSON.stringify(filterOptions, null, 2));

    const filter = filterOptions.filter || {};
    const sort = filterOptions.sort || { GoLiveDate: -1 }; // Default sort by GoLiveDate descending

    let page = parseInt(filterOptions.page, 10) || 1;
    let limit = parseInt(filterOptions.limit, 10);

    // If limit is not a number, or not provided, default to 10.
    // If limit is 0, it means fetch all.
    if (isNaN(limit)) {
      limit = 10; // Default limit
    }

    console.log('Service: getAllReleases - Effective Filter:', JSON.stringify(filter));
...
async function getAcademicYearsWithReleases() {
  try {
    ...
    if (!distinctAcademicYearIds || distinctAcademicYearIds.length === 0) {
      return []; // No academic years have releases
    }

    // Fetch the details of these academic years
    const academicYears = await AcademicYear.find({
      _id: { $in: distinctAcademicYearIds },
      status: { $in: ['Current', 'Next', 'Future'] } // Only include relevant academic years
    })
    .select('_id name code fullName status') // Select relevant fields
    .sort({ startDate: 1 }) // Sort by start date
    .lean();

    return academicYears;
  } catch (error) {
    console.error('Error fetching academic years with releases:', error);
    throw error;
  }
}

async function getReleasesByAcademicYearId(academicYearId) {
  try {
    console.log(`Finding releases for academic year ID: ${academicYearId}`);
    
    // Using $or to handle different ways AcademicYearID could be stored
    const releases = await Release.find({
      $or: [
        { AcademicYearID: academicYearId }, // Direct match
        { 'AcademicYearID._id': academicYearId }, // Object with _id property
        { AcademicYearID: { _id: academicYearId } } // Object with _id directly
      ],
      IsArchived: { $ne: true }, // Exclude archived releases
      Status: { $nin: ['Cancelled', 'Postponed'] } // Exclude releases that are not active
    })
    .select('_id ReleaseCode ReleaseType ReleaseNameDetails Status GoLiveDate') // Select relevant fields
    .sort({ GoLiveDate: 1 }) // Sort by go-live date
    .lean();
    return releases;
  } catch (error) {
    console.error(`Error fetching releases for academic year ID ${academicYearId}:`, error);
    throw error;
  }
}
