...
const createWorkflowActivity = async (activityData) => {
  try {
    return await prisma.bcrWorkflowActivity.create({
      data: {
        id: uuidv4(),
        bcrId: activityData.bcrId,
        fromPhaseId: activityData.fromPhaseId,
        toPhaseId: activityData.toPhaseId,
...
const getWorkflowHistory = async (bcrId) => {
  try {
    return await prisma.bcrWorkflowActivity.findMany({
      where: { bcrId },
      include: {
        Users: true
      },
